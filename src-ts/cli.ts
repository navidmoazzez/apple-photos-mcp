...
import { runTool, type AnyToolSpec, type ToolContext } from "./tools/kit.js";
...
export function binName(): string {
  ...
  return name.startsWith("apple-photos-cli") ? "apple-photos-cli" : "apple-photos-mcp";
}
...
function renderToolList(tools: AnyToolSpec[]): string {
  ...
  for (const tool of tools) {
    const mark =
      tool.risk === "read" ? " " : tool.risk === "destructive" ? "!" : "*";
    lines.push(`  ${mark} ${commandName(tool.name).padEnd(width)}${tool.title}`);
  }
  lines.push(``);
  lines.push(`  * changes the library   ! cannot be undone from here, needs --confirm`);
  lines.push(``);
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with APPLE_PHOTOS_READ_ONLY=1.`
  ...
  let client: PythonBridge | undefined;
  try {
    ...
    client = new PythonBridge(config);
    const ctx: ToolContext = { bridge: client, config, guard };

    // runTool applies the same guard the MCP path applies, so --confirm,
    // READ_ONLY, ALLOW_DESTRUCTIVE and the audit log behave identically here.
    emit(await runTool(spec, args as Record<string, unknown>, ctx), format, select);
    return 0;
  } catch (error) {
